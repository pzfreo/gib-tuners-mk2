...
**DD cut and wheel geometry are generated by an external gear calculator tool.** Do not modify these values in `parameters.py` directly.

The external tool generates:
- `config/<profile>/worm_gear.json` - All gear parameters including DD bore dimensions
- `config/<profile>/wheel_*.step` - Wheel geometry with DD bore cut
- `config/<profile>/worm_*.step` - Worm geometry

The DD cut parameters in `parameters.py` (`DDCutParams` for wheel bore and string post) must match the values in `worm_gear.json`. These are loaded by `create_default_config()` from the JSON file.

**To change DD dimensions:** Update the gear calculator tool configuration and regenerate the STEP/JSON files. Then verify `parameters.py` defaults match (or rely on JSON loading to override them).

**Current DD constraint (bh profile):**
- Bore diameter: 3.5mm (from `features.wheel.bore_diameter_mm`)
- M2 screw (2.0mm OD) through 2.5mm across-flats leaves only 0.25mm wall
- This is marginal for the string post tapped section - do not reduce further
